...
from gen.generate_capabilities import FeatureInfo, generate_capabilities_py, parse_capabilities
...
def generate_output_files(
    out_dir: Path, generator: Generator, feature_infos: Dict[str, FeatureInfo]
) -> None:
    ...
    for f in static_dir.iterdir():
        if not f.suffix == ".py":
            continue
        with open(f, encoding="utf-8") as input, open(
            out_dir.joinpath(f.name), "w", newline="\n", encoding="utf-8"
        ) as output:
            output.write(
                """\
...
if __name__ == "__main__":
    ...
    _input_dir = Path(_args.input)
    _generator = create_generator(_input_dir / "metaModel.json")
    _feature_infos = parse_capabilities(Path(_input_dir / "capabilities.json"))

    generate_output_files(Path(_args.output), _generator, _feature_infos)
