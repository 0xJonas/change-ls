...
def test_parse_maptype() -> None:
    ...
    res = AnyType.from_json(test_json1)
    assert res == AnyType("map",
                          MapType(MapKeyType("string"),
    ...
    test_json2 = {
        ...
        "key": {
            "kind": "base",
            "name": "thing" # wrong literal type here
        },
        "value": {
            "kind": "base",
            "name": "integer"
    ...
    assert res == AnyType("map",
                          MapType(ReferenceType("test_ref"),
                                  AnyType("base", BaseType("integer"))))
...
def test_parse_andtype() -> None:
    ...
    res = AnyType.from_json(test_json)
    assert res == AnyType("and", AndType((
...
def test_parse_tupletype() -> None:
    ...
    res = AnyType.from_json(test_json)
    assert res == AnyType("tuple", TupleType((
        AnyType("base", BaseType("string")),
        AnyType("array", ArrayType(AnyType("base", BaseType("string"))))
    )))
...
def test_parse_structureliteraltype() -> None:
    ...
    assert res1 == AnyType("literal", StructureLiteralType(StructureLiteral(
        "A test StructureLiteral",
        (Property("A test Property", "test", False, False, "0.1.1", AnyType("base", BaseType("boolean"))),),
        True,
        "0.1.0"
    )))
    ...
    assert res2 == AnyType("literal", StructureLiteralType(StructureLiteral(
        None,
