...
from lsprotocol.types import ErrorCodes
...
class MockLSProtocol(LSProtocol):
    ...
    def __init__(
        ...
        super().__init__(
            lsprotocol.converters.get_converter(), request_handler, notification_handler
        )
...
def _empty_request_handler(method: str, params: Any) -> Any:
    return None


def _empty_notification_handler(method: str, params: Any) -> None:
    return None
...
async def test_send_request_receive_response() -> None:
    ...
    client.send_request(MockRequest(0, "test"), future)
...
async def test_send_request_receive_error() -> None:
    def server_request_handler(method: str, params: Any) -> None:
...
async def test_send_notification() -> None:
    ...
    def server_notification_handler(method: str, params: Any) -> None:
        future.set_result(True)

    client = MockLSProtocol(_empty_request_handler, _empty_notification_handler)
    server = MockLSProtocol(_empty_request_handler, server_notification_handler)

    client.send_notification(MockNotification("test"))
