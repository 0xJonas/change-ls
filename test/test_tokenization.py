...
async def test_tokenization_no_whitespace() -> None:
    ...
    install_language(
        language_id="mock", extensions=[".mock"], grammar=grammar, allow_overwrite=True
    )
    text = """\
function test begin
    a = b + c
...
"""
    ...
    assert len(tokens) == 9
    assert tokens[0] == SyntacticToken(
        "function", 0, {"source.mock", "meta.function", "storage.type.function"}
    )
    assert tokens[1] == SyntacticToken(
        "test", 9, {"source.mock", "meta.function", "entity.name.function"}
    )
    assert tokens[2] == SyntacticToken(
        "begin", 14, {"source.mock", "meta.function", "keyword.other.begin"}
    )
    assert tokens[3] == SyntacticToken(
        "a",
        24,
        {"source.mock", "meta.function", "variable.other"},
    )
    assert tokens[4] == SyntacticToken(
        "=", 26, {"source.mock", "meta.function", "keyword.operator.assignment"}
    )
    ...
    assert tokens[6] == SyntacticToken(
        "+", 30, {"source.mock", "meta.function", "keyword.operator.arithmetic"}
    )
    assert tokens[7] == SyntacticToken(
        "c",
        32,
        {"source.mock", "meta.function", "variable.other"},
    )
    assert tokens[8] == SyntacticToken(
        "end", 34, {"source.mock", "meta.function", "keyword.other.end"}
    )
...
"""
    ...
    assert tokens[9] == SyntacticToken(
        "=", 26, {"source.mock", "meta.function", "keyword.operator.assignment"}
    )
    ...
    assert tokens[13] == SyntacticToken(
        "+", 30, {"source.mock", "meta.function", "keyword.operator.arithmetic"}
    )
    ...
    assert tokens[17] == SyntacticToken(
        "end", 34, {"source.mock", "meta.function", "keyword.other.end"}
    )
